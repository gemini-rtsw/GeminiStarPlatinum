...
**Bridge flags:** `--source mock|epics` (default `mock`), `--host`/`--port` (default `127.0.0.1:9100`), `--rate` (default 20 Hz). For `--source epics`, the CA search mode is one of: nothing (UDP broadcast, finds a local TestIOC), `--ca_addr "<ip>"` (broadcast at a known host), or `--name_servers "10.26.70.200:5064 10.26.70.200:5065"` (TCP name resolution through the workstation tunnel, for the real TCS). `--ca_addr` and `--name_servers` are mutually exclusive and error if both are passed. These flags work by setting `EPICS_CA_*` env vars in `main()`, which libca reads once at context creation — **the `import epics` inside `EpicsSource.__init__` must stay lazy**, or the vars land too late and both flags silently fall back to broadcast. `EpicsSource` prints a per-PV `[ OK ] <pv> = <value>` / `[FAIL]` report on startup; `tools/test_listen.py` is the same check standalone.
